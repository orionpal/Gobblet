...
class GobbletGUI(Frame):
    ...
    def updateInv(self):
        for i in range(3):
            print(self.Game.PlayerW.getTopPiece(i+1).Size)
            self.Winv[i].config(image=self.piecePics[('W', self.Game.PlayerW.getTopPiece(i+1).Size)], width=75, height=75)
            self.Binv[i].config(image=self.piecePics[('B', self.Game.PlayerB.getTopPiece(i+1).Size)], width=75, height=75)
        pass
    def updateHold(self):
